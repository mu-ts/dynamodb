import {
  DynamoDBClient, ServiceInputTypes, ServiceOutputTypes,  
...
export class Client {
  ...
  private readonly client: Promise<DynamoDBClient>

  private constructor() {
    this.logger = LoggerService.named(this.constructor.name)
    this.client = new Promise((resolve) => resolve(new DynamoDBClient({ region: process.env.REGION || process.env.AWS_REGION || process.env.AWS_LAMBDA_REGION })))
  ...
  public async send<Input extends ServiceInputTypes, Output extends ServiceOutputTypes>(command: Input): Promise<Output> {
    ...
    const client: DynamoDBClient = await this.client
